...
class DrawingFlowable(Flowable):
    ...
    def draw(self):
        ...
        offset = (
            (available_width - self.width) / 2 if available_width > self.width else 0
        )
    ...
    settings = db.get(
        "image_settings",
        {
            ...
            "last_move_light": [
                ...
                90,
                15,
            ],  # Ultima mossa su casa chiara (verde chiaro)
            "last_move_dark": [
                70,
                40,
                40,
            ],  # Ultima mossa su casa scura (rosso/mattone)
...
def ask_rgb(label, defaults):
    ...
    r = dgt(
        prompt=_("  Rosso % [{d}]: ").format(d=defaults[0]),
        kind="i",
        imin=0,
        imax=100,
        default=defaults[0],
    ...
    g = dgt(
        prompt=_("  Verde % [{d}]: ").format(d=defaults[1]),
        kind="i",
        imin=0,
        imax=100,
        default=defaults[1],
    ...
    b = dgt(
        prompt=_("  Blu % [{d}]: ").format(d=defaults[2]),
        kind="i",
        imin=0,
        imax=100,
        default=defaults[2],
...
def image_settings_menu():
    ...
    settings["pieces_white"] = ask_rgb(
        _("Pezzi Bianchi (Riempimento)"), settings["pieces_white"]
    )
    settings["pieces_white_stroke"] = ask_rgb(
        _("Pezzi Bianchi (Bordi/Rifiniture)"),
        settings.get("pieces_white_stroke", [0, 0, 0]),
    )

    settings["pieces_black"] = ask_rgb(
        _("Pezzi Neri (Riempimento)"), settings["pieces_black"]
    )
    settings["pieces_black_stroke"] = ask_rgb(
        _("Pezzi Neri (Bordi/Rifiniture)"),
        settings.get("pieces_black_stroke", [100, 100, 100]),
    )

    settings["margin"] = ask_rgb(
        _("Sfondo Margine (Coordinate)"), settings.get("margin", [20, 20, 20])
    )
    settings["coord"] = ask_rgb(
        _("Testo Coordinate"), settings.get("coord", [90, 90, 90])
    )
    settings["inner_border"] = ask_rgb(
        _("Bordo Interno"), settings.get("inner_border", [100, 100, 100])
    )
    settings["outer_border"] = ask_rgb(
        _("Bordo Esterno"), settings.get("outer_border", [0, 0, 0])
    )

    settings["last_move_light"] = ask_rgb(
        _("Evidenzia Ultima Mossa (Casa Chiara)"),
        settings.get("last_move_light", [100, 100, 0]),
    )
    settings["last_move_dark"] = ask_rgb(
        _("Evidenzia Ultima Mossa (Casa Scura)"),
        settings.get("last_move_dark", [80, 80, 0]),
    )
    settings["check_color"] = ask_rgb(
        _("Colore Scacco Re"), settings.get("check_color", [100, 0, 0])
    )

    settings["piece_stroke_width"] = dgt(
        prompt=_("\nSpessore bordo pezzi (pixel) [{d}]: ").format(
            d=settings.get("piece_stroke_width", 1)
        ),
        kind="f",
        fmin=0.0,
        fmax=5.0,
        default=settings.get("piece_stroke_width", 1.3),
    )
...
def generate_custom_svg(board, node=None, orientation=None, override_size=None):
    ...
    svg_data = chess.svg.board(
        ...
        colors={
            ...
            "inner border": ib,
            "outer border": ob,
            "square light lastmove": lml,
            "square dark lastmove": lmd,
            "check": chk,
    ...
    def colorize_white(match):
        ...
        text = text.replace('fill="#fff"', f'fill="{pw}"').replace(
            'fill="#ffffff"', f'fill="{pw}"'
        )
        text = text.replace("fill:#ffffff", f"fill:{pw}").replace(
            "fill:#fff", f"fill:{pw}"
        )
        # Sostituisce i bordi e dettagli (che di default sono neri)
        text = text.replace('stroke="#000"', f'stroke="{pws}"').replace(
            'stroke="#000000"', f'stroke="{pws}"'
        )
        text = text.replace("stroke:#000000", f"stroke:{pws}").replace(
            "stroke:#000", f"stroke:{pws}"
        )
        return text
    ...
    def colorize_black(match):
        ...
        text = text.replace('fill="#000"', f'fill="{pb}"').replace(
            'fill="#000000"', f'fill="{pb}"'
        )
        text = text.replace("fill:#000000", f"fill:{pb}").replace(
            "fill:#000", f"fill:{pb}"
        )

        # IL FIX È QUI: Sostituisce i bordi ESTERNI (che di default sono neri)
        text = text.replace('stroke="#000"', f'stroke="{pbs}"').replace(
            'stroke="#000000"', f'stroke="{pbs}"'
        )
        text = text.replace("stroke:#000000", f"stroke:{pbs}").replace(
            "stroke:#000", f"stroke:{pbs}"
        )

        # Sostituisce le rifiniture INTERNE (che di default sono bianche)
        text = text.replace('stroke="#fff"', f'stroke="{pbs}"').replace(
            'stroke="#ffffff"', f'stroke="{pbs}"'
        )
        text = text.replace("stroke:#ffffff", f"stroke:{pbs}").replace(
            "stroke:#fff", f"stroke:{pbs}"
        )
        text = text.replace('fill="#fff"', f'fill="{pbs}"').replace(
            'fill="#ffffff"', f'fill="{pbs}"'
        )
        text = text.replace("fill:#ffffff", f"fill:{pbs}").replace(
            "fill:#fff", f"fill:{pbs}"
        )

        # Gestione speciale per i cavalli neri (hanno dettagli grigio #ececec di default)
        text = text.replace('stroke="#ececec"', f'stroke="{pbs}"').replace(
            "stroke:#ececec", f"stroke:{pbs}"
        )
        text = text.replace('fill="#ececec"', f'fill="{pbs}"').replace(
            "fill:#ececec", f"fill:{pbs}"
        )
    ...
    svg_data = re.sub(
        r'<g id="white-[^"]+".*?</g>', colorize_white, svg_data, flags=re.DOTALL
    )
    svg_data = re.sub(
        r'<g id="black-[^"]+".*?</g>', colorize_black, svg_data, flags=re.DOTALL
    )
    ...
    arrow_pattern = re.compile(
        r'(<line[^>]*class="arrow"[^>]*>|<polygon[^>]*class="arrow"[^>]*>|<circle[^>]*class="arrow"[^>]*>)'
    )
    ...
    if arrows_str:
        ...
        if insert_pos == -1:
            insert_pos = svg_data.find("</svg>")
        svg_data = svg_data[:insert_pos] + arrows_str + svg_data[insert_pos:]

    # 5. Bordi e Margini
    svg_data = re.sub(
        r'class="square light"', f'class="square light" fill="{sl}"', svg_data
    )
    svg_data = re.sub(
        r'class="square dark"', f'class="square dark" fill="{sd}"', svg_data
    )
    svg_data = svg_data.replace('class="margin"', f'class="margin" fill="{mrg}"')
    svg_data = svg_data.replace(
        'class="inner-border"', f'class="inner-border" stroke="{ib}"'
    )
    svg_data = svg_data.replace(
...
def export_board_pdf(board, node=None):
    ...
    try:
        ...
        story = [
            DrawingFlowable(drawing, svg_size, svg_size, scale=scale_factor),
            Spacer(1, 1 * cm),
        ]
        if user_comment:
            ...
            story.extend(
                [Paragraph(formatted_comment, comment_style), Spacer(1, 0.5 * cm)]
            )
