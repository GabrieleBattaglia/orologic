...
def verbalize_drawings(node):
    ...
    for a in arrows:
        if len(a) >= 5:
            ...
            sq1 = a[1:3]
            sq2 = a[3:5]
            lines.append(_("Freccia da {sq1} ad {sq2} ({col})").format(
    ...
    for c in circles:
        if len(c) >= 3:
            color = c[0]
            sq1 = c[1:3]
            lines.append(_("Cerchio su {sq} ({col})").format(
                sq=verbalize_square(sq1), 
                col=verbalize_color(color)
            ))
    return ", ".join(lines)
...
def drawing_menu(game, node):
    ...
    while True:
        ...
        choice = dgt(prompt=_("\nMenu Disegno (.d): [A]ggiungi, [L]ista, [C]ancella da questa mossa, [P]ulisci tutto, [.] Esci: "), kind="s", default=".").upper().strip()
        
        if choice == ".":
            break
            
        elif choice == "A":
            target = dgt(prompt=_("Inserisci casa (es. e4 per cerchio) o case (es. e4.f6 per freccia): "), kind="s", default="").lower().strip()
            if not target:
                continue
            ...
            col_choice = dgt(prompt=_("Scegli colore: [V]erde, [R]osso, [B]lu, [G]iallo: "), kind="s", default="V").upper().strip()
            ...
            if len(parts) == 1:
                ...
                print(_("Hai impostato un cerchio su {sq} di colore {col}.").format(sq=verbalize_square(sq), col=verbalize_color(color_code)))
            else:
                ...
                print(_("Hai impostato la freccia che parte da {sq1}, punta a {sq2} ed è di colore {col}.").format(sq1=verbalize_square(sq1), sq2=verbalize_square(sq2), col=verbalize_color(color_code)))
            ...
            conf = dgt(prompt=_("Sei sicuro di voler cancellare TUTTI i disegni dell'intero PGN? (S/N): "), kind="s", default="N").upper().strip()
            if conf == "S":
                count = clear_all_drawings(game)
                is_modified = True
                print(_("Pulizia completata. Cancellati disegni in {n} mosse.").format(n=count))
                Acusticator(["g4", 0.1, 0, config.VOLUME, "c4", 0.2, 0, config.VOLUME], kind=1)
