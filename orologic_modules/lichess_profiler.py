...
def search_player():
    print(_("\n=== RICERCA GIOCATORE ==="))
    print(_("Digita il nome utente. Usa il tasto Backspace per correggere, INVIO per confermare, ESC per annullare."))
    print(_("Seleziona uno dei suggerimenti scrivendolo per intero. Se non compare, premetti '=' al nome (es: =MioNome)."))
    ...
    while True:
        ...
        if c == '\x1b': # ESC
            ...
            exact_match = next((res.get("name") for res in results if res.get("name").lower() == term.lower()), None)
            if exact_match:
                return exact_match
            ...
            print(_("\nIl termine inserito '{t}' non corrisponde esattamente a un suggerimento. Affina la ricerca o usa '=' per forzare.").format(t=term))
...
def format_profile(profile):
    ...
    p.append(_("Bio: {b}").format(b=profile.get("profile", {}).get("bio", _("Non specificata"))))
    ...
    if t > 0:
        ...
        pd = (d / t) * 100
        pl = (l / t) * 100
        p.append(_("Partite totali: {t} (Vinte: {w} [{pw:.1f}%], Patte: {d} [{pd:.1f}%], Perse: {l} [{pl:.1f}%])").format(
            t=t, w=w, pw=pw, d=d, pd=pd, l=l, pl=pl))
    else:
        p.append(_("Partite totali: {t} (Vinte: {w}, Patte: {d}, Perse: {l})").format(t=t, w=w, d=d, l=l))
    ...
    if total_seconds > 0:
        ...
        time_parts = []
        if y > 0: time_parts.append(_("{y} anni").format(y=y) if y != 1 else _("1 anno"))
        if mo > 0: time_parts.append(_("{mo} mesi").format(mo=mo) if mo != 1 else _("1 mese"))
        if wk > 0: time_parts.append(_("{wk} settimane").format(wk=wk) if wk != 1 else _("1 settimana"))
        if dy > 0: time_parts.append(_("{dy} giorni").format(dy=dy) if dy != 1 else _("1 giorno"))
        if hr > 0: time_parts.append(_("{hr} ore").format(hr=hr) if hr != 1 else _("1 ora"))
        if mi > 0: time_parts.append(_("{mi} minuti").format(mi=mi) if mi != 1 else _("1 minuto"))
        
        p.append(_("Tempo di gioco totale: {t_str}").format(t_str=", ".join(time_parts)))
    else:
    ...
    for perf_name, perf_data in perfs.items():
        if perf_name in ["blitz", "rapid", "classical", "bullet", "puzzle", "correspondence"]:
            games = perf_data.get("games", 0)
            if games > 0:
                p.append(f"{perf_name.capitalize()}: {perf_data.get('rating', '?')} ({games} partite)")
...
def send_message(username, token):
    ...
    msg = input(_("\nScrivi il messaggio per {u} (lascia vuoto per annullare): ").format(u=username)).strip()
    if not msg:
        return
...
def download_games(username, token):
    ...
    if enter_escape(_("Premi INVIO per scaricare l'intero database, oppure ESC per impostare filtri di ricerca: ")):
        # Nessun filtro
        filters = {}
        is_partial = False
    else:
        ...
        # Colore
        col = key(_("Vuoi partite giocate col (b)ianco, (n)ero o (e)ntrambi? (b/n/e): ")).lower()
        if col == 'b': filters['color'] = 'white'
        elif col == 'n': filters['color'] = 'black'
        
        # Risultato
        res = key(_("Risultato: (v)ittoria, (p)areggio, (s)confitta o (t)utte? (v/p/s/t): ")).lower()
        if res in ['v', 'p', 's']: filters['result'] = res
        
        # Range Elo
        if enter_escape(_("Vuoi specificare un range di Elo per l'avversario? (INVIO per si', ESC per no): ")):
        ...
        # ECO
        if enter_escape(_("Vuoi specificare un codice ECO? (INVIO per si', ESC per no): ")):
            eco = input(_("Codice ECO (es. B10): ")).strip().upper()
            if eco: filters['eco'] = eco
            ...
            for line in resp:
                ...
                if is_partial:
                    ...
                    if 'result' in filters:
                        ...
                        player_color = "white" if game.get("players", {}).get("white", {}).get("user", {}).get("id") == username.lower() else "black"
                        
                        if res_filter == 'v':
                            if winner != player_color: keep = False
                        elif res_filter == 's':
                            if winner is None or winner == player_color: keep = False
                        elif res_filter == 'p':
                            if winner is not None: keep = False
                            
                    # Elo filter
                    if keep and 'elo' in filters:
                        min_e, max_e = filters['elo']
                        opp_color = "black" if game.get("players", {}).get("white", {}).get("user", {}).get("id") == username.lower() else "white"
                        opp_elo = game.get("players", {}).get(opp_color, {}).get("rating", 0)
...
def choose_game_for_analysis(games_list):
    ...
    while True:
        ...
        for i, g in enumerate(page_games):
            idx = start_idx + i
            w = g.get("players", {}).get("white", {}).get("user", {}).get("name", "Anonimo")
            b = g.get("players", {}).get("black", {}).get("user", {}).get("name", "Anonimo")
            res = ""
            if g.get("winner") == "white": res = "1-0"
            elif g.get("winner") == "black": res = "0-1"
            else: res = "1/2-1/2"
            
            eco = g.get("opening", {}).get("eco", "")
            date = datetime.datetime.fromtimestamp(g.get("createdAt", 0)/1000).strftime("%Y-%m-%d")
        ...
        print(_("\n--- Partite Trovate (Pagina {c}/{t}) ---").format(c=current_page+1, t=(total + page_size - 1)//page_size))
        ...
        if scelta == ".":
            ...
            print(_("Premi Invio per continuare e, quando sarai nel menu principale di Easyfish, digita '.pg' per incollare e analizzare la partita."))
            input()
