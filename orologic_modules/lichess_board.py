...
def save_lichess_game(game_state, result_str="*"):
    ...
    if pgn_text:
        try:
            ...
            if game:
                ...
                print(_("La partita e' stata scaricata da Lichess e salvata nella cartella PGN."))
                return
        except Exception:
    ...
    try:
        pgn_handler.SaveGameToFile(game)
        print(_("La partita e' stata salvata nella cartella PGN (ricostruzione locale)."))
        ...
        try:
            ...
            if msg == "EOF":
                ...
                if game_state.is_live:
                    sys.stdout.write('\r' + ' ' * 79 + '\r')
                    sys.stdout.write(_("{turn} gioca: {desc}\n").format(turn=turn_name, desc=desc))
...
def spectate_game(game_id, token=None):
    ...
    while True:
        ...
        if cmd == ".1":
            ...
            print(_("\nTempo {p}: {t}").format(p=game_state.white_player, t=format_time(w_time)))
        elif cmd == ".2":
            _w_time, b_time = game_state.get_clocks()
            Acusticator(['b6', 0.14, 1, config.VOLUME], kind=1, adsr=[0, 0, 100, 100])
            print(_("\nTempo {p}: {t}").format(p=game_state.black_player, t=format_time(b_time)))
        elif cmd == ".3":
            w_time, b_time = game_state.get_clocks()
            Acusticator(['e7', 0.14, 0, config.VOLUME], kind=1, adsr=[0, 0, 100, 100])
            print(_("\nTempo {p}: {t}").format(p=game_state.white_player, t=format_time(w_time)))
            print(_("Tempo {p}: {t}").format(p=game_state.black_player, t=format_time(b_time)))
        elif cmd == ".4":
            w_time, b_time = game_state.get_clocks()
            ...
            player = game_state.white_player if game_state.board.turn == chess.WHITE else game_state.black_player
...
def send_action(game_id, token, action, uci=None, chat_text=None):
    ...
    if action == "chat" and chat_text:
        data = urllib.parse.urlencode({"room": "player", "text": chat_text}).encode("utf-8")
    try:
    ...
    while True:
        ...
        try:
            ...
            if msg.get("type") == "eof":
                ...
                if "aiLevel" in w: w_name = f"Stockfish level {w['aiLevel']}"
                w_rat = w.get("rating", "?")
                
                b_name = b.get("name", b.get("id", _("Anonimo")))
                if "aiLevel" in b: b_name = f"Stockfish level {b['aiLevel']}"
                b_rat = b.get("rating", "?")
                ...
                sys.stdout.write("\r" + " " * 79 + "\r")
                sys.stdout.write(_("Partita: {wp} vs {bp}\n").format(wp=game_state.white_player, bp=game_state.black_player))
                col_str = _("Bianco") if game_state.my_color == chess.WHITE else _("Nero")
                ...
                if len(new_moves) > current_ply:
                    ...
                    for i in range(current_ply, len(new_moves)):
                        ...
                        if is_white_turn == (game_state.my_color == chess.WHITE):
                            ...
                            Acusticator(["c5", 0.05, 0, config.VOLUME, "e5", 0.05, 0, config.VOLUME, "g5", 0.05, 0, config.VOLUME], kind=1, adsr=[0, 0, 100, 5])
                            Acusticator([1000.0, 0.05, 0, config.VOLUME], kind=1, adsr=[0, 0, 100, 0]) # Extra beep for my turn
                ...
                if game_state.board.turn == game_state.my_color and game_state.premove:
                    try:
                        ...
                        uci = move.uci()
                        sys.stdout.write("\r" + " " * 79 + "\r")
                        sys.stdout.write(_("Eseguo premove: {m}...\n").format(m=game_state.premove))
                        threading.Thread(target=send_action, args=(game_state.game_id, game_state.token, "move", uci), daemon=True).start()
                    except Exception:
                        sys.stdout.write("\r" + " " * 79 + "\r")
                        sys.stdout.write(_("La premove ({m}) non e' piu' valida.\n").format(m=game_state.premove))
                ...
                if user.lower() != game_state.my_username.lower() and user != "lichess":
                    ...
                    Acusticator([800.0, 0.1, 0, config.VOLUME, 1200.0, 0.1, 0, config.VOLUME], kind=2)
                    refresh_line()
                ...
                if gone and not getattr(game_state, 'opponent_gone_announced', False):
                    ...
                    Acusticator([300.0, 0.2, 0, config.VOLUME, 400.0, 0.2, 0, config.VOLUME], kind=1)
                    refresh_line()
                else:
                    if gone:
...
def play_game(game_id, token, username):
    ...
    while True:
        ...
        if user_input == "." or user_input.lower() == ".q":
            if enter_escape(_("Vuoi davvero disconnetterti da questa partita? Non la abbandonerai su Lichess (Invio = Si', Esc = No): ")):
                stop_event.set()
                break
            continue
        ...
        if user_input.startswith("_"):
            ...
            if chat_text:
                threading.Thread(target=send_action, args=(game_id, token, "chat", None, chat_text), daemon=True).start()
                print(_("Messaggio inviato."))
            continue
            
        cmd = user_input.lower()
        if cmd == "resign":
            if enter_escape(_("Sei sicuro di voler abbandonare (resign)? (Invio = Si', Esc = No): ")):
            ...
            if game_state.claim_win_in_seconds and game_state.claim_win_in_seconds > 0:
                print(_("Troppo presto. Potrai reclamare la vittoria tra {s} secondi.").format(s=game_state.claim_win_in_seconds))
            else:
        ...
        if cmd == ".1":
            ...
            if is_valid:
                if game_state.board.turn == game_state.my_color:
                    uci = move.uci()
                    threading.Thread(target=send_action, args=(game_id, token, "move", uci), daemon=True).start()
                else:
                    game_state.premove = raw_input
                    print(_("Premove impostata: {m}. Verra' giocata al tuo turno.").format(m=raw_input))
            else:
