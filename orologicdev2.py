# OROLOGIC - DEV2 - Data di concepimento: 14/02/2025 by Gabriele Battaglia & AIs
...
VERSION="4.10.39"
RELEASE_DATE=datetime.datetime(2025,8,3,21,14)
...
def AnalisiAutomatica(pgn_game):
	...
	for i, node in enumerate(mainline_nodes):
		...
		centipawn_loss = 0
		classification = "Mossa Normale"  # Impostazione predefinita

		best_pov_score = best_alternative['score'].pov(turn)
		played_pov_score = eval_after_move.pov(turn)

		# Caso 1: La mossa migliore portava a un matto a nostro favore.
		if best_pov_score.is_mate() and best_pov_score.mate() > 0:
			# Se la mossa giocata non è un matto a nostro favore, è uno Svarione.
			if not (played_pov_score.is_mate() and played_pov_score.mate() > 0):
				classification = "Svarione"
				# Usiamo un valore simbolico altissimo per indicare un 'mancato matto'.
				# Questo garantisce che la variante venga sempre mostrata.
				centipawn_loss = 5000
			else:
				# Entrambe le mosse portano al matto. Calcoliamo la perdita in base
				# a quanto è più lento il matto giocato.
				score_best = best_pov_score.score(mate_score=30000)
				score_played = played_pov_score.score(mate_score=30000)
				centipawn_loss = score_best - score_played # Questo valore sarà piccolo

		# Caso 2: La mossa migliore non era matto, ma la nostra mossa ci fa subire matto.
		elif not best_pov_score.is_mate() and (played_pov_score.is_mate() and played_pov_score.mate() < 0):
			classification = "Svarione"
			centipawn_loss = 5000 # Valore simbolico per 'subire un matto'

		# Caso 3: Nessun matto immediato in gioco. Logica standard basata sui centipedoni.
		else:
			score_best = best_pov_score.score(mate_score=30000)
			score_played = played_pov_score.score(mate_score=30000)
		...
		# Ora, applichiamo le soglie di classificazione standard se non è già stato definito uno Svarione
		if classification == "Svarione":
			pass # La classificazione è già definitiva
		elif centipawn_loss >= soglia_svarione:
			classification = "Svarione"
		elif centipawn_loss >= soglia_errore:
			classification = "Errore"
		elif centipawn_loss >= soglia_inesattezza:
			classification = "Inesattezza"
		elif node.move.uci() == best_alternative['move'].uci():
			# La logica per "Mossa Buona" e "Mossa Geniale" rimane invariata
			if len(analysis_before) > 1:
				...
				if second_best_score is not None and (best_pov_score.score(mate_score=30000) - second_best_score) >= soglia_mossa_geniale_gap:
